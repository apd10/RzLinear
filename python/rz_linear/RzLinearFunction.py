...
class RzLinearFunction(torch.autograd.Function):
    ...
    def forward(ctx, input: torch.tensor, hashed_weight: torch.tensor,
                random_numbers: torch.tensor, output_dim, chunk_size, is_hnet, init_factor) -> torch.tensor:
        '''
            Args:
        ...
        if (M, K, N, H) in RzLinearFunction._memoize_dict and is_hnet is False:
            BLOCK_SIZE_M, BLOCK_SIZE_K, BLOCK_SIZE_N, num_warps, num_stages = RzLinearFunction._memoize_dict[(
                M, K, N, H)]
            output = rz_linear_forward_tl(input, hashed_weight, init_factor, M, K, N, H, R3, R2, R1, R0,
    ...
    def backward(ctx, grad):
        ...
        if (M, K, N, H) in RzLinearFunction._memoize_dict and is_hnet is False:
            BLOCK_SIZE_M, BLOCK_SIZE_K, BLOCK_SIZE_N, num_warps, num_stages = RzLinearFunction._memoize_dict[(
                M, K, N, H)]
            input_grad, weight_grad = rz_linear_backward_tl(input, hashed_weight, grad, init_factor, M, K, N, H, R3, R2, R1,

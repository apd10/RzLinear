    ...
    for batch_size in batch_sizes:
        for shape in shapes:
            ...
            for mem_size in mem_sizes:
                ...
                # warmup
                rz_linear_forward_tl(x, weight, init_factor, M, K, N, H, 
                                     R3, R2, R1, R0, allow_tf32=allow_tf32, allow_autotune=False)

                def bench():
                    for _ in range(ITERS):
                        if mode in ["forward+backward"]:
                            output = rz_linear_forward_tl(x, weight, init_factor, M, K, N, H, R3, R2, R1, R0,
                            ...
                            rz_linear_backward_tl(x, weight, output, init_factor, M, K, N, H, R3, R2, R1, R0,
                                                  allow_tf32=allow_tf32, allow_autotune=False,
                                                  BLOCK_SIZE_K=BLOCK_SIZE_K, BLOCK_SIZE_N=BLOCK_SIZE_N, BLOCK_SIZE_M=BLOCK_SIZE_M,
                                                  num_stages=num_stages, num_warps=num_warps)
